...
(function () {
    ...
    async function actionBuildWorkflow(args) {
        ...
        // ---- 单人强制（防多出人）----
        // 无 pose_file 且无显式双人意图时，正向注入单人限定、负面追加多人排除，
        // 避免“一个美女”之类的请求被模型画出 2 个或更多人。
        if (!poseFile) {
            const PAIR_HINTS = ['2girls', 'two girls', 'two women', 'two people', 'two persons', 'couple', 'pair', 'double', 'both', 'dual', 'twin', '双人', '两人', '一对', '二人'];
            const hay = (positive + ' ' + negative).toLowerCase();
            const isPair = PAIR_HINTS.some((k) => hay.includes(k));
            if (!isPair) {
                if (!/(^|[,\s])(1girl|solo|single person|one woman|one girl|alone)([,\s]|$)/i.test(positive)) {
                    positive = '1girl, solo, single person, only one woman, ' + positive;
                }
