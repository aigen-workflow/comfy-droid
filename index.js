...
(function () {
    ...
    const DEFAULT_SETTINGS = {
        ...
        comic_style: true,           // 为 true 时，出图默认注入“真人漫画”风格（写实脸+漫画渲染），用户指定其他风格时不注入
    };
    ...
    async function actionBuildWorkflow(args) {
        ...
        // ---- 默认真人漫画风格（可关：扩展设置 comic_style=false）----
        // 单图真人漫画：写实脸 + 漫画渲染。用户已明确指定其他风格（photorealistic/动漫等）时不重复注入。
        if (settings.comic_style) {
            const STYLE_OVERRIDE = ['photorealistic', 'photo', 'realistic photo', 'anime', 'manga', 'cartoon', '3d render', 'oil painting', 'watercolor'];
