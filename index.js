...
(function () {
    ...
    async function actionGenerateImage(args) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            // FAIL：优先尝试局部放大重绘（boxes 定位问题区域）；无 boxes 或重绘失败 → 换 seed 整图重试
            ...
            if (boxesMatch && res.images && res.images.length && settings.quality_inpaint !== false) {
                const repaired = await tryInpaintRepair(res.images[0].url, boxesMatch[1], maskMatch ? maskMatch[1] : null, a);
    ...
    // 对每个问题区域（repair_boxes）做「裁切 → 放大 ~6x → denoise 0.5 重绘 → 缩回 → 贴回」，
    // 最后 QualityGate 复审，PASS 即交付。
    async function tryInpaintRepair(imageUrl, boxesStr, maskFile, args) {
        const base = settings.comfy_endpoint.replace(/\/+$/, '');
        try {
            const boxes = String(boxesStr || '').split(';').filter(Boolean).map((b) => {
            ...
            if (!boxes.length) return { error: '无有效修复区域' };
            ...
            // 脸部修复提示词：中性、保留原身份（测试验证 denoise 0.5 + 中性词接缝最自然）
            ...
            let destRef = ['1', 0];
            let lastNode = null;
            const targetLongEdge = 800;  // 放大到长边约 800px（SD1.5 脸部舒适区）
            for (const box of boxes) {
                ...
                wf[sampId] = { class_type: 'KSampler', inputs: {
                    ...
                    denoise: 0.5,
                    model: ['4', 0], positive: ['6', 0], negative: ['7', 0], latent_image: [encId, 0],
                } };
            ...
            if (settings.quality_gate) {
                wf['30'] = { class_type: 'QualityGate', inputs: { image: [lastNode, 0], expected_people: inferExpectedPeople(args), min_body_kp: 10, min_face_kp: 20 } };
